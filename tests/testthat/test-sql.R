...
test_that("rush_sql executes the query", {
  script <- capture_script(rush_sql, query = "SELECT 1 AS x")
  expect_true(any(grepl(
    'result <- DBI::dbGetQuery\\(con, "SELECT 1 AS x"\\)',
    script
  )))
})

test_that("rush_sql registers CSV files as relations", {
  script <- capture_script(rush_sql, query = "SELECT * FROM a", file = "a.csv")
...
test_that("rush_sql handles stdin", {
  script <- capture_script(rush_sql, query = "SELECT * FROM stdin", file = "-")
...
test_that("rush_sql respects output_format", {
  script <- capture_script(rush_sql, query = "SELECT 1", output_format = "json")
