...
[devoutansi](https://github.com/coolbutuseless/devoutansi):

```bash
rush plot -x bill_length_mm -y body_mass_g penguins.csv
```

The output uses block characters and ANSI colors to approximate the scatter
plot directly in your terminal, with axis labels and tick marks.
...
```bash
rush plot -x bill_length_mm -y body_mass_g --width 60 penguins.csv
```

### `--output-format ansi` / `ascii` (`-O`)

When piping to another command or redirecting to a file, stdout is not a TTY
and rush defaults to PNG output. Use `-O ansi` to force terminal art
regardless:
