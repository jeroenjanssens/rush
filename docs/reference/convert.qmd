...
Use `--xml-root-name` and `--xml-row-name` to customize the XML structure:

```{bash}
rush convert --xml-root-name plants --xml-row-name observation --head 2 -o plants.xml penguins.csv
cat plants.xml
rm -f plants.xml
```
