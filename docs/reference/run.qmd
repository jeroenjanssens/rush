...
Read a file without a header row. Columns are auto-named `x1`, `x2`, ...:
...
### `--xml-root-name`, `--xml-row-name`

Customize the XML element names when reading or writing XML. Defaults are `data`
(root) and `row` (each record):
